...
unsafeValToValue :: HasCallStack => Val (Instr cp) t -> Un.Value (Un.Op nop)
...
valToOpOrValue ::
     forall cp t nop.
     Val (Instr cp) t
  -> Either (Operation (Instr cp)) (Un.Value (Un.Op nop))
valToOpOrValue = \case
...
instrToOps :: Instr cp inp out -> [Un.Op nop]
instrToOps instr = Un.Op <$> handleInstr instr
  where
    handleInstr :: Instr cp inp out -> [Un.Instr nop]
    ...
    handleInstr i@(PUSH val) = handle i
      where
        handle :: Instr cp inp (t ': s) -> [Un.Instr nop]
    ...
    handleInstr i@(NONE) = handle i
      where
        handle :: Instr cp inp ('T_option a ': inp) -> [Un.Instr nop]
    ...
    handleInstr i@(LEFT) = handle i
      where
        handle :: Instr cp (a ': s) ('T_or a b ': s) -> [Un.Instr nop]
    ...
    handleInstr i@(RIGHT) = handle i
      where
        handle :: Instr cp (b ': s) ('T_or a b ': s) -> [Un.Instr nop]
    ...
    handleInstr i@(NIL) = handle i
      where
        handle :: Instr cp s ('T_list p ': s) -> [Un.Instr nop]
    ...
    handleInstr i@(EMPTY_SET) = handle i
      where
        handle :: Instr cp s ('T_set e ': s) -> [Un.Instr nop]
    ...
    handleInstr i@(EMPTY_MAP) = handle i
      where
        handle :: Instr cp s ('T_map a b ': s) -> [Un.Instr nop]
    ...
    handleInstr i@(LAMBDA l) = handle i
      where
        handle :: Instr cp s ('T_lambda i o ': s) -> [Un.Instr nop]
        ...
        handle _ = error "unexcepted call"
        convertLambdaBody :: Val (Instr cp) ('T_lambda i o) -> [Un.Op nop]
    ...
    handleInstr i@(CAST) = handle i
      where
        handle :: Instr cp (a ': s) (a ': s) -> [Un.Instr nop]
    ...
    handleInstr i@(UNPACK) = handle i
      where
        handle :: Instr cp ('T_c 'T_bytes ': s) ('T_option a ': s) -> [Un.Instr nop]
