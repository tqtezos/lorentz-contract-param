...
instance (RenderDoc op) => RenderDoc (InstrAbstract op) where
  renderDoc = \case
    ...
    PAIR ta va fa1 fa2    -> "PAIR" <+> renderDoc ta <+> renderDoc va <+> renderTwoAnnotations fa1 fa2
    CAR va fa             -> "CAR" <+> renderDoc va <+> renderDoc fa
    CDR va fa             -> "CDR" <+> renderDoc va <+> renderDoc fa
    LEFT ta va fa1 fa2 t  -> "LEFT" <+> renderDoc ta <+> renderDoc va <+> renderTwoAnnotations fa1 fa2 <+> renderDoc t
    RIGHT ta va fa1 fa2 t -> "RIGHT" <+> renderDoc ta <+> renderDoc va <+> renderTwoAnnotations fa1 fa2 <+> renderDoc t
    ...
    where
      ...
      renderTwoAnnotations ann1 ann2 =
        ...
        then renderAnn ann1 <+> renderAnn ann2
        else renderDoc ann1 <+> renderDoc ann2
